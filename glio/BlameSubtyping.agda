...
... | result ⟨ μ₁ , w′ , 𝓁ᶜ₁ ⟩ | ▹result μ≡μ₁ 𝓁ᶜ≡𝓁ᶜ₁ | ⊢ᵣresult ⊢μ₁ ⊢w′
  {- NOTE:
    ...
    we have 𝓁ᶜ ≾ 𝓁̂₁ .
  -}
  with castL μ₁ 𝓁ᶜ₁ 𝓁̂₁′ 𝓁̂₁ 𝓁̂₁′≾𝓁̂₁ | castL-state-idem {μ₁} {𝓁ᶜ₁} {𝓁̂₁′} {𝓁̂₁} 𝓁̂₁′≾𝓁̂₁
...   | result ⟨ μ₂ , _ , 𝓁ᶜ₂ ⟩ | ▹result μ₁≡μ₂ 𝓁ᶜ₁≡𝓁ᶜ₂
  with apply γ v w′ μ₂ 𝓁ᶜ₂ k | inspect (λ □ → apply □ v w′ μ₂ 𝓁ᶜ₂ k) γ
...     | result ⟨ μ₃ , v₁ , 𝓁ᶜ₃ ⟩ | [ eq₁ ]
  with castL μ₃ 𝓁ᶜ₃ 𝓁̂₂ 𝓁̂₂′ 𝓁̂₂≾𝓁̂₂′
...       | result ⟨ μ₄ , _ , 𝓁ᶜ₄ ⟩
  with castT μ₄ 𝓁ᶜ₄ T T′ v₁
...         | result ⟨ μ₅ , v₂ , 𝓁ᶜ₅ ⟩ =
  let 𝓁ᶜ₂≾𝓁₁ = subst (λ □ → (l̂ □) ≾ 𝓁̂₁′) (trans 𝓁ᶜ≡𝓁ᶜ₁ 𝓁ᶜ₁≡𝓁ᶜ₂) 𝓁ᶜ≾𝓁₁ in
  let μ≡μ₂ = trans μ≡μ₁ μ₁≡μ₂ in
  let ⊢μ₂ = subst (λ □ → □ ⊢ₛ □) μ≡μ₂ ⊢μ in
  let μ₂⊢γ = subst (λ □ → _ ∣ □ ⊢ₑ γ) μ≡μ₂ ⊢γ in
  let freshμ₂ = subst (λ □ → length □ ∉domₙ □) μ≡μ₂ fresh in
  let μ₂⊢v = subst (λ □ → □ ⊢ᵥ v ⦂ _) μ≡μ₂ ⊢v in
  let μ₂⊢w′ = subst (λ □ → □ ⊢ᵥ _ ⦂ _) μ₁≡μ₂ ⊢w′ in
  let 𝓁ᶜ₃≾𝓁₂ = apply-pc≾ {!!} ⊢μ₂ μ₂⊢γ freshμ₂ μ₂⊢v μ₂⊢w′ eq₁ in
    {!!}
...
... | v | ⊢v | w | ⊢w
  with castT μ₁ 𝓁ᶜ₁ T′ T w | ⊢castT {pc = 𝓁ᶜ₁} {T′} {T} ⊢μ₁ ⊢w
...   | result ⟨ μ′ , w′ , 𝓁ᶜ′ ⟩ | ⊢ᵣresult ⊢μ′ ⊢w′
  with castL μ′ 𝓁ᶜ′ 𝓁̂₁′ 𝓁̂₁ 𝓁̂₁′≾𝓁̂₁
...     | result ⟨ μ″ , _ , 𝓁ᶜ″ ⟩ = {!!}
