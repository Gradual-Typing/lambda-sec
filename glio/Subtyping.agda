...
-- 𝒱-pres-pc≲ {γ = γ} {μ₁ = μ₁} {𝓁ᶜ₁ = 𝓁ᶜ₁} (suc k) _ _ (⊢if {x = x} _ ⊢M ⊢N _) eq with nth γ x
-- 𝒱-pres-pc≲ {γ = γ} {μ₁ = μ₁} {𝓁ᶜ₁ = 𝓁ᶜ₁} (suc k) _ _ (⊢if _ ⊢M ⊢N _) eq | just V-true
--   with 𝒱 γ _ ⊢M μ₁ 𝓁ᶜ₁ k | inspect (λ ⊢M → 𝒱 γ _ ⊢M μ₁ 𝓁ᶜ₁ k) ⊢M
-- 𝒱-pres-pc≲ {μ₁ = μ₁} {𝓁ᶜ₁ = 𝓁ᶜ₁} (suc k) _ _ (⊢if _ ⊢M ⊢N _) eq | just V-true | error _ | _ = ⊥-elim (error≢result eq)
-- 𝒱-pres-pc≲ {μ₁ = μ₁} {𝓁ᶜ₁ = 𝓁ᶜ₁} (suc k) _ _ (⊢if {𝓁̂₂ = 𝓁̂₂} {𝓁̂₂′} _ ⊢M ⊢N _) eq | just V-true | result ⟨ μ′ , vₘ , 𝓁ᶜ′ ⟩ | _
--   with castL μ′ 𝓁ᶜ′ 𝓁̂₂ (𝓁̂₂ ⋎ 𝓁̂₂′) (𝓁̂≾𝓁̂⋎𝓁̂′ _ _) | inspect (λ □ → castL □ 𝓁ᶜ′ 𝓁̂₂ (𝓁̂₂ ⋎ 𝓁̂₂′) (𝓁̂≾𝓁̂⋎𝓁̂′ _ _)) μ′
-- 𝒱-pres-pc≲ {μ₁ = μ₁} {𝓁ᶜ₁ = 𝓁ᶜ₁} (suc k) _ _ (⊢if _ ⊢M ⊢N _) eq
--   | just V-true | result ⟨ μ′ , vₘ , 𝓁ᶜ′ ⟩ | _ | timeout | _ = ⊥-elim (timeout≢result eq)
-- 𝒱-pres-pc≲ {μ₁ = μ₁} {𝓁ᶜ₁ = 𝓁ᶜ₁} (suc k) _ _ (⊢if _ ⊢M ⊢N _) eq
--   | just V-true | result ⟨ μ′ , vₘ , 𝓁ᶜ′ ⟩ | _ | error _ | _ = ⊥-elim (error≢result eq)
-- 𝒱-pres-pc≲ {μ₁ = μ₁} {𝓁ᶜ₁ = 𝓁ᶜ₁} (suc k) _ _ (⊢if {T = T} {T′} {T″} _ ⊢M ⊢N _) eq
...
-- 𝒱-pres-pc≲ {μ₁ = μ₁} {𝓁ᶜ₁ = 𝓁ᶜ₁} (suc k) 𝓁ᶜ₁≾𝓁₁ ⊢γ (⊢if {T = T} {T′} {T″} {𝓁̂₂ = 𝓁̂₂} {𝓁̂₂′} _ ⊢M ⊢N _) eq
--   | just V-true | result ⟨ μ′ , vₘ , 𝓁ᶜ′ ⟩ | [ eq₁ ] | result ⟨ μ″ , _ , 𝓁ᶜ″ ⟩ | [ eq₂ ] | result ⟨ μ₂′ , _ , 𝓁ᶜ₂′ ⟩ | [ eq₃ ] =
--   let 𝓁ᶜ′≾𝓁₂ = 𝒱-pres-pc≲ k 𝓁ᶜ₁≾𝓁₁ ⊢γ ⊢M eq₁ in
...
--     𝓁≾𝓁₁→𝓁≾𝓁₁⋎𝓁₂ 𝓁ᶜ₂≾𝓁₂
-- 𝒱-pres-pc≲ {γ = γ} {μ₁ = μ₁} {𝓁ᶜ₁ = 𝓁ᶜ₁} (suc k) 𝓁ᶜ≾𝓁₁ _ (⊢if _ ⊢M ⊢N _) eq | just V-false
--   with 𝒱 γ _ ⊢N μ₁ 𝓁ᶜ₁ k | inspect (λ ⊢N → 𝒱 γ _ ⊢N μ₁ 𝓁ᶜ₁ k) ⊢N
-- 𝒱-pres-pc≲ {μ₁ = μ₁} {𝓁ᶜ₁ = 𝓁ᶜ₁} (suc k) _ _ (⊢if _ ⊢M ⊢N _) eq | just V-false | error _ | _ = ⊥-elim (error≢result eq)
-- 𝒱-pres-pc≲ {μ₁ = μ₁} {𝓁ᶜ₁ = 𝓁ᶜ₁} (suc k) _ _ (⊢if {𝓁̂₂ = 𝓁̂₂} {𝓁̂₂′} _ ⊢M ⊢N _) eq | just V-false | result ⟨ μ′ , vₙ , 𝓁ᶜ′ ⟩ | _
--   with castL μ′ 𝓁ᶜ′ 𝓁̂₂′ (𝓁̂₂ ⋎ 𝓁̂₂′) (𝓁̂≾𝓁̂′⋎𝓁̂ _ _) | inspect (λ □ → castL □ 𝓁ᶜ′ 𝓁̂₂′ (𝓁̂₂ ⋎ 𝓁̂₂′) (𝓁̂≾𝓁̂′⋎𝓁̂ _ _)) μ′
-- 𝒱-pres-pc≲ {μ₁ = μ₁} {𝓁ᶜ₁ = 𝓁ᶜ₁} (suc k) _ _ (⊢if _ ⊢M ⊢N _) eq
...
-- 𝒱-pres-pc≲ {μ₁ = μ₁} {𝓁ᶜ₁ = 𝓁ᶜ₁} {𝓁ᶜ₂} (suc k) 𝓁ᶜ₁≾𝓁₁ ⊢γ (⊢if {T = T} {T′} {T″} {𝓁̂₂ = 𝓁̂₂} {𝓁̂₂′} _ ⊢M ⊢N _) eq
--   | just V-false | result ⟨ μ′ , vₙ , 𝓁ᶜ′ ⟩ | [ eq₁ ] | result ⟨ μ″ , _ , 𝓁ᶜ″ ⟩ | [ eq₂ ] | result ⟨ μ₂′ , _ , 𝓁ᶜ₂′ ⟩ | [ eq₃ ] =
--   let 𝓁ᶜ′≾𝓁₂′ = 𝒱-pres-pc≲ k 𝓁ᶜ₁≾𝓁₁ ⊢γ ⊢N eq₁ in
--   let 𝓁ᶜ′≡𝓁ᶜ″ = castL→𝓁ᶜ≡ (𝓁̂≾𝓁̂′⋎𝓁̂ 𝓁̂₂′ 𝓁̂₂) eq₂ in
