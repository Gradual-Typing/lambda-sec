...
𝒱-pres-pc≲ : ∀ {Γ γ μ₁ μ₂ 𝓁ᶜ₁ 𝓁ᶜ₂ 𝓁̂₁ 𝓁̂₂ M v T}
  ...
  → (l̂ 𝓁ᶜ₂) ≾ 𝓁̂₂
𝒱-pres-pc≲ 0 _ _ ()
𝒱-pres-pc≲ {γ = γ} (suc k) 𝓁ᶜ₁≾𝓁₁ (⊢` {x = x} eq) eq₁
  with nth γ x
... | nothing = ⊥-elim (error≢result eq₁)
... | just _ rewrite sym (result≡→𝓁ᶜ≡ eq₁) = 𝓁ᶜ₁≾𝓁₁
𝒱-pres-pc≲ (suc k) 𝓁ᶜ₁≾𝓁₁ ⊢tt eq rewrite sym (result≡→𝓁ᶜ≡ eq) = 𝓁ᶜ₁≾𝓁₁
𝒱-pres-pc≲ (suc k) 𝓁ᶜ₁≾𝓁₁ ⊢true eq rewrite sym (result≡→𝓁ᶜ≡ eq) = 𝓁ᶜ₁≾𝓁₁
𝒱-pres-pc≲ (suc k) 𝓁ᶜ₁≾𝓁₁ ⊢false eq rewrite sym (result≡→𝓁ᶜ≡ eq) = 𝓁ᶜ₁≾𝓁₁
𝒱-pres-pc≲ {Γ} {γ} {μ₁} {μ₂} {𝓁ᶜ₁} (suc k) 𝓁ᶜ₁≾𝓁₁ (⊢let {T = T} {T′} {M = M} {N} {𝓁̂₁} {𝓁̂₂} {𝓁̂₃} ⊢M ⊢N T′≲T) eq
  with 𝒱 γ M ⊢M μ₁ 𝓁ᶜ₁ k | inspect (λ □ → 𝒱 γ M □ μ₁ 𝓁ᶜ₁ k) ⊢M
... | error _ | _ = ⊥-elim (error≢result eq)
... | result ⟨ μ′ , v′ , 𝓁ᶜ′ ⟩ | [ eq₁ ]
  with castT μ′ 𝓁ᶜ′ T′ T v′ | inspect (λ v → castT μ′ 𝓁ᶜ′ T′ T v) v′
...   | timeout | _ = ⊥-elim (timeout≢result eq)
...   | error _ | _ = ⊥-elim (error≢result eq)
...   | result ⟨ μ″ , v″ , 𝓁ᶜ″ ⟩ | [ eq₂ ] =
  let 𝓁ᶜ′≾𝓁₂ = 𝒱-pres-pc≲ k 𝓁ᶜ₁≾𝓁₁ ⊢M eq₁ in
  let 𝓁ᶜ′≡𝓁ᶜ″ = castT→𝓁ᶜ≡ {μ₁ = μ′} {𝓁ᶜ₁ = 𝓁ᶜ′} {T₁ = T′} {T} v′ eq₂ in
  let 𝓁ᶜ″≾𝓁₂ = subst (λ □ → l̂ □ ≾ 𝓁̂₂) 𝓁ᶜ′≡𝓁ᶜ″ 𝓁ᶜ′≾𝓁₂ in
    𝒱-pres-pc≲ k 𝓁ᶜ″≾𝓁₂ ⊢N eq
𝒱-pres-pc≲ {γ = γ} {μ₁ = μ₁} {𝓁ᶜ₁ = 𝓁ᶜ₁} (suc k) _ (⊢if {x = x} _ ⊢M ⊢N _) eq with nth γ x
𝒱-pres-pc≲ {γ = γ} {μ₁ = μ₁} {𝓁ᶜ₁ = 𝓁ᶜ₁} (suc k) _ (⊢if _ ⊢M ⊢N _) eq | just V-true
  with 𝒱 γ _ ⊢M μ₁ 𝓁ᶜ₁ k | inspect (λ ⊢M → 𝒱 γ _ ⊢M μ₁ 𝓁ᶜ₁ k) ⊢M
𝒱-pres-pc≲ {μ₁ = μ₁} {𝓁ᶜ₁ = 𝓁ᶜ₁} (suc k) _ (⊢if _ ⊢M ⊢N _) eq | just V-true | error _ | _ = ⊥-elim (error≢result eq)
𝒱-pres-pc≲ {μ₁ = μ₁} {𝓁ᶜ₁ = 𝓁ᶜ₁} (suc k) _ (⊢if {𝓁̂₂ = 𝓁̂₂} {𝓁̂₂′} _ ⊢M ⊢N _) eq | just V-true | result ⟨ μ′ , vₘ , 𝓁ᶜ′ ⟩ | _
  with castL μ′ 𝓁ᶜ′ 𝓁̂₂ (𝓁̂₂ ⋎ 𝓁̂₂′) (𝓁̂≾𝓁̂⋎𝓁̂′ _ _) | inspect (λ □ → castL □ 𝓁ᶜ′ 𝓁̂₂ (𝓁̂₂ ⋎ 𝓁̂₂′) (𝓁̂≾𝓁̂⋎𝓁̂′ _ _)) μ′
𝒱-pres-pc≲ {μ₁ = μ₁} {𝓁ᶜ₁ = 𝓁ᶜ₁} (suc k) _ (⊢if _ ⊢M ⊢N _) eq
  | just V-true | result ⟨ μ′ , vₘ , 𝓁ᶜ′ ⟩ | _ | timeout | _ = ⊥-elim (timeout≢result eq)
𝒱-pres-pc≲ {μ₁ = μ₁} {𝓁ᶜ₁ = 𝓁ᶜ₁} (suc k) _ (⊢if _ ⊢M ⊢N _) eq
  | just V-true | result ⟨ μ′ , vₘ , 𝓁ᶜ′ ⟩ | _ | error _ | _ = ⊥-elim (error≢result eq)
𝒱-pres-pc≲ {μ₁ = μ₁} {𝓁ᶜ₁ = 𝓁ᶜ₁} (suc k) _ (⊢if {T = T} {T′} {T″} _ ⊢M ⊢N _) eq
  | just V-true | result ⟨ μ′ , vₘ , 𝓁ᶜ′ ⟩ | _ | result ⟨ μ″ , _ , 𝓁ᶜ″ ⟩ | _
  with castT μ″ 𝓁ᶜ″ T T″ vₘ | inspect (λ v → castT μ″ 𝓁ᶜ″ T T″ v) vₘ
𝒱-pres-pc≲ {μ₁ = μ₁} {𝓁ᶜ₁ = 𝓁ᶜ₁} (suc k) _ (⊢if _ ⊢M ⊢N _) eq
  | just V-true | result ⟨ μ′ , vₘ , 𝓁ᶜ′ ⟩ | _ | result ⟨ μ″ , _ , 𝓁ᶜ″ ⟩ | _ | timeout | _ = ⊥-elim (timeout≢result eq)
𝒱-pres-pc≲ {μ₁ = μ₁} {𝓁ᶜ₁ = 𝓁ᶜ₁} (suc k) _ (⊢if _ ⊢M ⊢N _) eq
  | just V-true | result ⟨ μ′ , vₘ , 𝓁ᶜ′ ⟩ | _ | result ⟨ μ″ , _ , 𝓁ᶜ″ ⟩ | _ | error _ | _ = ⊥-elim (error≢result eq)
𝒱-pres-pc≲ {μ₁ = μ₁} {𝓁ᶜ₁ = 𝓁ᶜ₁} (suc k) 𝓁ᶜ₁≾𝓁₁ (⊢if {T = T} {T′} {T″} {𝓁̂₂ = 𝓁̂₂} {𝓁̂₂′} _ ⊢M ⊢N _) eq
  | just V-true | result ⟨ μ′ , vₘ , 𝓁ᶜ′ ⟩ | [ eq₁ ] | result ⟨ μ″ , _ , 𝓁ᶜ″ ⟩ | [ eq₂ ] | result ⟨ μ₂′ , _ , 𝓁ᶜ₂′ ⟩ | [ eq₃ ] =
  let 𝓁ᶜ′≾𝓁₂ = 𝒱-pres-pc≲ k 𝓁ᶜ₁≾𝓁₁ ⊢M eq₁ in
  let 𝓁ᶜ′≡𝓁ᶜ″ = castL→𝓁ᶜ≡ (𝓁̂≾𝓁̂⋎𝓁̂′ 𝓁̂₂ 𝓁̂₂′) eq₂ in
  let 𝓁ᶜ″≡𝓁ᶜ₂′ = castT→𝓁ᶜ≡ {μ₁ = μ″} {𝓁ᶜ₁ = 𝓁ᶜ″} {T₁ = T} {T″} vₘ eq₃ in
  let 𝓁ᶜ₂′≡𝓁ᶜ₂ = result≡→𝓁ᶜ≡ eq in
  let 𝓁ᶜ₂≾𝓁₂ = subst (λ □ → l̂ □ ≾ 𝓁̂₂) (trans 𝓁ᶜ′≡𝓁ᶜ″ (trans 𝓁ᶜ″≡𝓁ᶜ₂′ 𝓁ᶜ₂′≡𝓁ᶜ₂)) 𝓁ᶜ′≾𝓁₂ in
    𝓁≾𝓁₁→𝓁≾𝓁₁⋎𝓁₂ 𝓁ᶜ₂≾𝓁₂
𝒱-pres-pc≲ {γ = γ} {μ₁ = μ₁} {𝓁ᶜ₁ = 𝓁ᶜ₁} (suc k) 𝓁ᶜ≾𝓁₁ (⊢if _ ⊢M ⊢N _) eq | just V-false
  with 𝒱 γ _ ⊢N μ₁ 𝓁ᶜ₁ k | inspect (λ ⊢N → 𝒱 γ _ ⊢N μ₁ 𝓁ᶜ₁ k) ⊢N
𝒱-pres-pc≲ {μ₁ = μ₁} {𝓁ᶜ₁ = 𝓁ᶜ₁} (suc k) _ (⊢if _ ⊢M ⊢N _) eq | just V-false | error _ | _ = ⊥-elim (error≢result eq)
𝒱-pres-pc≲ {μ₁ = μ₁} {𝓁ᶜ₁ = 𝓁ᶜ₁} (suc k) _ (⊢if {𝓁̂₂ = 𝓁̂₂} {𝓁̂₂′} _ ⊢M ⊢N _) eq | just V-false | result ⟨ μ′ , vₙ , 𝓁ᶜ′ ⟩ | _
  with castL μ′ 𝓁ᶜ′ 𝓁̂₂′ (𝓁̂₂ ⋎ 𝓁̂₂′) (𝓁̂≾𝓁̂′⋎𝓁̂ _ _) | inspect (λ □ → castL □ 𝓁ᶜ′ 𝓁̂₂′ (𝓁̂₂ ⋎ 𝓁̂₂′) (𝓁̂≾𝓁̂′⋎𝓁̂ _ _)) μ′
𝒱-pres-pc≲ {μ₁ = μ₁} {𝓁ᶜ₁ = 𝓁ᶜ₁} (suc k) _ (⊢if _ ⊢M ⊢N _) eq
  | just V-false | result ⟨ μ′ , vₙ , 𝓁ᶜ′ ⟩ | _ | timeout | _ = ⊥-elim (timeout≢result eq)
𝒱-pres-pc≲ {μ₁ = μ₁} {𝓁ᶜ₁ = 𝓁ᶜ₁} (suc k) _ (⊢if _ ⊢M ⊢N _) eq
  | just V-false | result ⟨ μ′ , vₙ , 𝓁ᶜ′ ⟩ | _ | error _ | _ = ⊥-elim (error≢result eq)
𝒱-pres-pc≲ {μ₁ = μ₁} {𝓁ᶜ₁ = 𝓁ᶜ₁} (suc k) _ (⊢if {T = T} {T′} {T″} _ ⊢M ⊢N _) eq
  | just V-false | result ⟨ μ′ , vₙ , 𝓁ᶜ′ ⟩ | _ | result ⟨ μ″ , _ , 𝓁ᶜ″ ⟩ | _
  with castT μ″ 𝓁ᶜ″ T′ T″ vₙ | inspect (λ v → castT μ″ 𝓁ᶜ″ T′ T″ v) vₙ
𝒱-pres-pc≲ {μ₁ = μ₁} {𝓁ᶜ₁ = 𝓁ᶜ₁} (suc k) _ (⊢if _ ⊢M ⊢N _) eq
  | just V-false | result ⟨ μ′ , vₙ , 𝓁ᶜ′ ⟩ | _ | result ⟨ μ″ , _ , 𝓁ᶜ″ ⟩ | _ | timeout | _ = ⊥-elim (timeout≢result eq)
𝒱-pres-pc≲ {μ₁ = μ₁} {𝓁ᶜ₁ = 𝓁ᶜ₁} (suc k) _ (⊢if _ ⊢M ⊢N _) eq
  | just V-false | result ⟨ μ′ , vₙ , 𝓁ᶜ′ ⟩ | _ | result ⟨ μ″ , _ , 𝓁ᶜ″ ⟩ | _ | error _ | _ = ⊥-elim (error≢result eq)
𝒱-pres-pc≲ {μ₁ = μ₁} {𝓁ᶜ₁ = 𝓁ᶜ₁} {𝓁ᶜ₂} (suc k) 𝓁ᶜ₁≾𝓁₁ (⊢if {T = T} {T′} {T″} {𝓁̂₂ = 𝓁̂₂} {𝓁̂₂′} _ ⊢M ⊢N _) eq
  | just V-false | result ⟨ μ′ , vₙ , 𝓁ᶜ′ ⟩ | [ eq₁ ] | result ⟨ μ″ , _ , 𝓁ᶜ″ ⟩ | [ eq₂ ] | result ⟨ μ₂′ , _ , 𝓁ᶜ₂′ ⟩ | [ eq₃ ] =
  let 𝓁ᶜ′≾𝓁₂′ = 𝒱-pres-pc≲ k 𝓁ᶜ₁≾𝓁₁ ⊢N eq₁ in
  let 𝓁ᶜ′≡𝓁ᶜ″ = castL→𝓁ᶜ≡ (𝓁̂≾𝓁̂′⋎𝓁̂ 𝓁̂₂′ 𝓁̂₂) eq₂ in
  let 𝓁ᶜ″≡𝓁ᶜ₂′ = castT→𝓁ᶜ≡ {μ₁ = μ″} {𝓁ᶜ₁ = 𝓁ᶜ″} {T₁ = T′} {T″} vₙ eq₃ in
  let 𝓁ᶜ₂′≡𝓁ᶜ₂ = result≡→𝓁ᶜ≡ eq in
  let 𝓁ᶜ₂≾𝓁₂′ = subst (λ □ → l̂ □ ≾ 𝓁̂₂′) (trans 𝓁ᶜ′≡𝓁ᶜ″ (trans 𝓁ᶜ″≡𝓁ᶜ₂′ 𝓁ᶜ₂′≡𝓁ᶜ₂)) 𝓁ᶜ′≾𝓁₂′ in
    subst (λ □ → l̂ 𝓁ᶜ₂ ≾ □) (⋎-comm 𝓁̂₂′ 𝓁̂₂) (𝓁≾𝓁₁→𝓁≾𝓁₁⋎𝓁₂ 𝓁ᶜ₂≾𝓁₂′)

𝒱-pres-pc≲ {γ = γ} {μ₁ = μ₁} {𝓁ᶜ₁ = 𝓁ᶜ₁} {𝓁ᶜ₂} (suc k) 𝓁ᶜ₁≾𝓁₁ (⊢get {x = x} _) eq with nth γ x
... | nothing = ⊥-elim (error≢result eq)
... | just (V-ref ⟨ n , 𝓁₁ , 𝓁₂ ⟩) with lookup μ₁ ⟨ n , 𝓁₁ , 𝓁₂ ⟩
...   | just ⟨ T′ , v ⟩ = {!!}
...   | nothing = ⊥-elim (error≢result eq)
