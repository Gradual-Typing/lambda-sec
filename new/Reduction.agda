...
data _∣_∣_∣_—→_∣_ : Term → Heap → HeapContext → StaticLabel → Term → Heap → Set where
  ...
  assign-static : ∀ {L M μ Σ pc ℓ}
      ----------------------------------------- AssignStatic
    → L [ ℓ ]:= M ∣ μ ∣ Σ ∣ pc —→ L [ ℓ ]:=✓ M ∣ μ

  assign?-ok : ∀ {V M μ Σ pc a ℓ ℓ₁}
    → key _≟_ μ a ≡ just ⟨ V , ℓ₁ ⟩
    → pc ≼ ℓ₁
      ----------------------------------------------------------------------------- AssignNSUSuccess
    → (addr a of ℓ) :=? M ∣ μ ∣ Σ ∣ pc —→ (addr a of ℓ) [ ℓ₁ ]:=✓ M ∣ μ
  ...
  assign : ∀ {V μ Σ pc a ℓ ℓ₁}
    → Value V
      --------------------------------------------------------------------- Assign
    → (addr a of ℓ) [ ℓ₁ ]:=✓ V ∣ μ ∣ Σ ∣ pc —→ $ tt of low ∣ ⟨ a , V , ℓ₁ ⟩ ∷ μ
  ...
  assign-cast : ∀ {V W μ Σ pc A B g₁ g₂ ℓ} {c : Cast (Ref A of g₁) ⇒ (Ref B of g₂)}
    → Value V → Value W
    → (i : Inert c)
      --------------------------------------------------------------------------------------------- AssignCast
    → (V ⟨ c ⟩) [ ℓ ]:=✓ W ∣ μ ∣ Σ ∣ pc —→ elim-ref-proxy V W i _[ ℓ ]:=✓_ {- V := (W ⟨ in/c c ⟩) -} ∣ μ
