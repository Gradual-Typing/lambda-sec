...
open Eq using (_≡_; refl; sym; cong; cong₂; subst)
open import Statics
...
fundamental {t = ((t₁ / 𝓁₁) ⇒ (t₂ / 𝓁₂))} σ₁ σ₂ (val (ƛ N / 𝓁)) σ₁≈σ₂ = λ {⇓-val ⇓-val → fundamental-lemma {N = N} σ₁ σ₂ σ₁≈σ₂ }
  where
  fundamental-lemma : ∀ {Γ ζ t₁ t₂ 𝓁₁ 𝓁₂ 𝓁 N}
    → (σ₁ σ₂ : Subst Γ ∅)
    → Γ ⊢ σ₁ ≈ₛ⦅ ζ ⦆ σ₂
    → ((t₁ / 𝓁₁) ⇒ (t₂ / 𝓁₂)) / 𝓁 ⦂ (ƛ (substₑ (exts σ₁) N) / 𝓁) ≈ᵥ⦅ ζ ⦆ (ƛ (substₑ (exts σ₂) N) / 𝓁)
  fundamental-lemma {Γ} {ζ} {t₁} {t₂} {𝓁₁} {𝓁₂} {𝓁} {N} σ₁ σ₂ σ₁≈σ₂ = λ 𝓁⊑ζ {v₁′} {v₂′} v₁′≈v₂′ →
    λ {(⇓-app {V = v₁″} ⇓-val ⇓-val ⇓v₁″) (⇓-app {V = v₂″} ⇓-val ⇓-val ⇓v₂″) →
      let σ₁•≈σ₂• = ≈ₛ-• {Γ} {t₁} {𝓁₁} {ζ} {val v₁′} {val v₂′} σ₁ σ₂ σ₁≈σ₂ (≈ᵥ→≈ₑ v₁′≈v₂′) in
      let σ₁•N⇓v₁″ = (subst (λ □ → □ ⇓ v₁″) (subst-ext σ₁ N (val v₁′)) ⇓v₁″) in
      let σ₂•N⇓v₂″ = (subst (λ □ → □ ⇓ v₂″) (subst-ext σ₂ N (val v₂′)) ⇓v₂″) in
